...
class FatTree:
    ...
    def expStressTest(self, demandList, results, x, alg, dir):
        percentPlrList = []
        plr1List = []
        plr2List = []
        SUList = []
        for dId in demandList:
            ...
            for i in range(len(percent)):
                if demandList.index(dId) / len(demandList) == percent[i]:
                    suList = self.currentSU()
                    datPath = dir + '/' + alg +'_su'+ str(percent[i]) + '-' + x + '.dat'
                    plrList = self.currentPlr()
                    datPathPlr = dir + '/' + alg +'_plr'+ str(percent[i]) + '-' + x + '.dat'
                    sorList = self.currentSor()
                    dataPathSor = dir + '/' + alg +'_sor'+ str(percent[i]) + '-' + x + '.dat'
            if suList != None:
                f = open(datPath, 'wb')
